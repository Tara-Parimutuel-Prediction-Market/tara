...
export class TelegramService {
  ...
  constructor(
    ...
    @InjectRepository(Market) private readonly marketRepository: Repository<Market>,
  ...
  async sendMessage(chatId: number, text: string, keyboard?: TelegramInlineKeyboard): Promise<void> {
    try {
  ...
  async sendPredictionRequest(chatId: number): Promise<void> {
    ...
    if (activeMarkets.length === 0) {
      await this.sendMessage(chatId, '🎯 No active markets right now. Check back later!')
      return
    }
  ...
  private async sendMiniAppLink(chatId: number, marketId: string, messageId: number): Promise<void> {
    ...
    await this.sendMessage(chatId, '🚀 Opening prediction interface...', keyboard)
  }

  private async handleQuickBet(callbackData: string, userId: number): Promise<void> {
    ...
    this.logger.log(`Quick bet request: ${marketId} - ${outcome} by user ${userId}`)
  }
  ...
  private async sendPortfolioView(chatId: number): Promise<void> {
    ...
    if (!user) {
      await this.sendMessage(chatId, '📊 No portfolio found. Start making predictions!')
      return
    }
  ...
  private formatMarketAnnouncement(market: Market): string {
    ...
    return `🎯 <b>New Market Available</b>\n\n` +
           `📊 ${market.title}\n` +
           `🎲 ${outcomes}\n` +
           `⏰ Closes: ${closesAt}\n\n` +
  ...
  async broadcastMessage(message: string, targetUsers?: string[]): Promise<void> {
    ...
    this.logger.log(`Broadcast message sent to ${users.length} users in ${batches} batches`)
  }
}
