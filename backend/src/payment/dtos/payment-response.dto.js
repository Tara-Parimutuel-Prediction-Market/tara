function _ts_decorate(decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for(var i = decorators.length - 1; i >= 0; i--)if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
}
function _ts_metadata(k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
}
import { NumberField } from "../../../decorators/field.decorators.js";
import { StringField } from "../../../decorators/field.decorators.js";
import { StringFieldOptional } from "../../../decorators/field.decorators.js";
export class PaymentResponseDto {
}
_ts_decorate([
    StringField(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "transactionId", void 0);
_ts_decorate([
    StringField(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "status", void 0);
_ts_decorate([
    NumberField(),
    _ts_metadata("design:type", Number)
], PaymentResponseDto.prototype, "amount", void 0);
_ts_decorate([
    StringField(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "currency", void 0);
_ts_decorate([
    StringField(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "accountNumber", void 0);
_ts_decorate([
    StringField(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "accountName", void 0);
_ts_decorate([
    StringFieldOptional(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "inquiryId", void 0);
_ts_decorate([
    StringFieldOptional(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "txnStatusId", void 0);
_ts_decorate([
    StringField(),
    _ts_metadata("design:type", String)
], PaymentResponseDto.prototype, "message", void 0);

//# sourceMappingURL=payment-response.dto.js.map